...
* No effect of robot's f0 mean on human's f0 mean (regardless of *turn* being in the model)

```{r}
summary(f0 <- lmer(f0mean ~ robf0meanc : condition : turn + (1 + condition | speaker), dac))
...
```{r}
summary(f2 <- lmer(f0sd ~ robf0sdc : condition   + (1 + condition | speaker), dac))
...
* Like for f0 SD, the coefficient of variation diverges in the Gaze Aversion condition.
...
* Intimacy reduces f0 mean and robot (condition nor robot'S f0) doesn't affect this effect.
* Intensity is not affected by intimacy.
...
* F0 SD decreases with increase in intimacy
* Intensity is not affected by intimacy
...
* F0 CV is not affeced by intimacy

```{r}
summary(f1 <- lmer(f0CV ~ intimc  + (1 | speaker), dac))
...
```{r}
summary(intel1 <- lmer(f0sd ~ robf0sdc : IOc : condition : turn + (1 + condition | speaker), dac %>% filter(!grepl("Robot", speaker))))

summary(no1 <- lmer(f0sd ~ robf0sdc : condition : turn + (1 + condition | speaker), dac %>% filter(!grepl("Robot", speaker))))
anova(intel1, no1)
```
...
d <- dac %>%
  filter(!grepl("Robot", speaker)) %>% 
  select(f0sd, robf0sdc, Agreec, Agreeableness, condition, speaker, turn) %>% 
  na.omit()

summary(agree1 <- lmer(f0sd ~ robf0sdc : Agreec : condition : turn + (1 +condition| speaker), d))

summary(no1 <- lmer(f0sd ~ robf0sdc : condition : turn + (1+condition | speaker), d))
anova(agree1, no1)
...
d <- dac %>%
  filter(!grepl("Robot", speaker)) %>% 
  select(f0sd, robf0sdc, Conscc, condition, speaker, turn)%>% 
  na.omit()

summary(con1 <- lmer(f0sd ~ robf0sdc : Conscc : condition : turn+ (1 + condition | speaker), d))
...
d <- dac %>%
  filter(!grepl("Robot", speaker)) %>% 
  select(f0sd, f0sdZ, robf0sdZ,  robf0sdc, robPrevf0sd, ESc, EmotionalStability, condition, turn, speaker, intimc) %>% 
  na.omit()
...
```{r}
summary(cf1 <- lmer(f0sd ~ robf0sdc : ConvQualc + (1 | speaker), dac))

summary(no1 <- lmer(f0sd ~ robf0sdc : turn + (1 | speaker), dac %>% filter(!grepl("Robot", speaker))))
anova(cf1, no1)
```
...
```{r}
summary(hl1 <- lmer(f0sd ~ robf0sdc : EvalRobotc + (1  | speaker), dac %>% filter(!grepl("Robot", speaker))))
...
```{r}
summary(ft1 <- lmer(f0Diff ~ condition * turn + (1 + condition | speaker), dac))
```
