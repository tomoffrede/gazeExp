...
```{r}
summary(f1 <- lmer(f0mean ~ intimc + intensMeanC + (1  | speaker), dac))
...
```{r}
summary(fit.totalEffect <- lmer(f0mean ~ intimc + (1|speaker), dac))
summary(fit.mediator <- lmer(intensMean ~ intimc + (1|speaker), dac))
summary(fit.both <- lmer(f0mean ~ intimc + intensMeanC + (1|speaker), dac))
