...
folderTGs <- "C:/Users/offredet/Documents/1HU/ExperimentEyes/Data/TGsWithPausesBasedOnPraatDetectSilences-xxx/"
...
filesTG <- list.files(folderTGs, "\\.TextGrid")
...
for(i in 1:nrow(files)){
  tg <- tg.read(paste0(folderTGs, files$filesTG[[i]]), encoding=detectEncoding(paste0(folderTGs, files$filesTG[[i]])))
...
save(f0, file=paste0(folder, "f0-VoicedPeriodsNotIPUs.RData"))
...
intCO <- read.csv(paste0(folderTGs, "intensityCO.csv"), sep="\t") %>% 
  filter(segment=="") %>% 
  select(File, start_segment, end_segment, dur_segment, intensity) %>% 
  rename(file=File)

intBL <- read.csv(paste0(folderTGs, "intensityBL.csv"), sep="\t") %>% 
  select(File, start_segment, end_segment, dur_segment, intensity) %>% 
  rename(file=File)
...
save(dac, file = paste0(folder, "dataConversation-VoicedPeriodsNotIPUs.RData"))
save(dab, file = paste0(folder, "dataBaseline-VoicedPeriodsNotIPUs.RData"))
