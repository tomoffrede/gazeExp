...
* `conditoion : intimacy`: f0 gets lower with higher intimacy only in the the gaze aversion condition.
See:
...
* `intimacy`: lower f0 for higher intimacy

Since in the intimacy ~ f0 graph f0 also seems to reduce (less strongly) with higher intimacy, I'd say we could report this model.
...
d <- dac %>%
  filter(!grepl("Robot", speaker)) %>% 
  select(f0mean, robf0Z, EmotionalStability, condition, speaker, intimZ) %>% 
  na.omit()
...
```{r}
summary(cf1 <- lmer(f0mean ~ robf0Z : PCConvQuality + (1 | speaker), dac %>% filter(!grepl("Robot", speaker))))
...
```{r}
summary(hl1 <- lmer(f0mean ~ robf0Z : PCRobotQuality + (1 | speaker), dac %>% filter(!grepl("Robot", speaker))))
