...
```{r}
summary(cf1 <- lmer(f0mean ~ robf0Normal : PCConvQuality + (1 + condition | speaker), dac %>% filter(!grepl("Robot", speaker))))

summary(no1 <- lmer(f0mean ~ robf0Normal + (1 + condition | speaker), dac %>% filter(!grepl("Robot", speaker))))
anova(cf1, no1)
```
...
```{r}
summary(hl1 <- lmer(f0mean ~ robf0Normal : PCRobotQuality + (1 + condition | speaker), dac %>% filter(!grepl("Robot", speaker))))
...
```{r}
summary(cq1 <- lmer(PCRobotQuality ~ condition + (1|speaker), dac))
```
