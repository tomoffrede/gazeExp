...
summary(f1 <- lmer(f0mean ~ robf0meanc : condition + (1 + condition | speaker), dac %>% filter(!grepl("Robot", speaker))))
...
```{r}
dat <- dac %>% 
  filter(abs(f0sd) < 30, abs(robf0sdc) < 30)

ggplot(dat, aes(robf0sdc, f0sd))+
  geom_point()+
  facet_wrap(~condition)+
  geom_smooth(method="lm")
```
...
summary(f1 <- lmer(f0sd ~ robf0sdc : condition + (1 + condition | speaker), dat %>% filter(!grepl("Robot", speaker))))
summary(f2 <- lmer(f0sd ~ robf0sdc : condition : turnNormal + (1 + condition | speaker), dat %>% filter(!grepl("Robot", speaker))))
anova(f1,f2)
...
summary(f2 <- lmer(f0mean ~ intimc : robf0meanc : condition + (1 + condition | speaker), dac %>% filter(!grepl("Robot", speaker))))
...
d <- dac %>%
  ...
  select(f0mean, robf0meanc, Agreec, Agreeableness, condition, speaker) %>% 
  na.omit()

summary(agree1 <- lmer(f0mean ~ robf0meanc : Agreec : condition + (1 +condition| speaker), d))

summary(no1 <- lmer(f0mean ~ robf0meanc : condition + (1+condition | speaker), d))
anova(agree1, no1)
```
#### Conscientiousness
...
d <- dac %>%
  filter(!grepl("Robot", speaker)) %>% 
  select(f0mean, robf0meanc, Conscc, condition, speaker) %>% 
  na.omit()
...
d <- dac %>%
  filter(!grepl("Robot", speaker)) %>% 
  select(f0mean, robf0meanc, robPrevf0mean, ESc, EmotionalStability, condition, speaker, intimc) %>% 
  na.omit()

summary(em1c <- lmer(f0mean ~ robf0meanc : ESc : condition + (1 + condition | speaker), d))
...
```{r}
summary(cf1 <- lmer(f0mean ~ robf0meanc : ConvQualc + (1 | speaker), dac %>% filter(!grepl("Robot", speaker))))

summary(no1 <- lmer(f0mean ~ robf0meanc + (1 | speaker), dac %>% filter(!grepl("Robot", speaker))))
anova(cf1, no1)
```
...
```{r}
summary(hl1 <- lmer(f0mean ~ robf0meanc : RobQualc + (1  | speaker), dac %>% filter(!grepl("Robot", speaker))))
...
summary(f2 <- lmer(f0mean ~ intimMean + robPrevf0meanMock : condition + (1 + condition | speaker), dac %>% filter(!grepl("Robot", speaker))))
```
...
summary(fb2 <- lmer(f0mean ~ robPrevf0mean : condition + (1 | speaker), dab))
```

Similar result with robot's *mock* f0.

```{r}
summary(fbM <- lmer(f0mean ~ robPrevf0meanMock : condition + (1 | speaker), dab))
```
