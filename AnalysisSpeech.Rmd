...
```
```{r}
ggplot(dac, aes(Conscientiousness, f0z))+
  geom_point()+
  facet_wrap(~condition)+
  geom_smooth(method = "lm")+
  ggtitle("Conscientiousness")

heatmap(dac$Conscientiousness)
...
summary(f1 <- lmer(f0z ~ IntellectOpenness : robf0Normal + robf0Normal : condition + (1 + condition | speaker), dac %>% filter(!grepl("Robot", speaker))))
```
