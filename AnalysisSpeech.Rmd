...
summary(f1 <- lmer(f0mean ~ robf0Z : condition + (1 + condition | speaker), dac %>% filter(!grepl("Robot", speaker))))
```

### Intimacy

* `condition : intimacy`: f0 gets lower with higher intimacy.

```{r}
summary(f1 <- lmer(f0mean ~ intimZ + (1 | speaker), dac %>% filter(!grepl("Robot", speaker))))
```
