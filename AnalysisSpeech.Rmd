...
```{r}
summary(f2 <- lmer(f0sd ~ robf0sdc : condition : turnNormal + (1 + condition | speaker) , dac %>% filter(!grepl("Robot", speaker))))
...
```{r}
summary(f1 <- lmer(f0CV ~ robf0CVc : condition + (1 + condition | speaker), dat %>% filter(!grepl("Robot", speaker))))
summary(f2 <- lmer(f0CV ~ robf0CVc : condition : turnNormal + (1 + condition | speaker), dat %>% filter(!grepl("Robot", speaker))))
anova(f1,f2)
...
```{r}
summary(f1 <- lmer(f0mean ~ intimc + intensMeanC + (1 | speaker), dac %>% filter(!grepl("Robot", speaker))))
...
```{r}
summary(f1 <- lmer(f0sd ~ intimc + intensMeanC + (1 | speaker), dac %>% filter(!grepl("Robot", speaker))))
...
d <- dac %>%
  filter(!grepl("Robot", speaker)) %>% 
  select(f0sd, robf0sdc, robPrevf0sd, ESc, EmotionalStability, condition, turnNormal, speaker, intimc) %>% 
  na.omit()

summary(em1c <- lmer(f0sd ~ robf0sdc : ESc : condition : turnNormal + (1 + condition | speaker), dac))
...
```{r}
summary(ex1 <- lmer(f0sd ~ robf0sdc : Extrac : condition :turnNormal+ (1 + condition | speaker), dac %>% filter(!grepl("Robot", speaker))))
...
```{r}
summary(hl1 <- lmer(f0sd ~ robf0sdc : RobQualc : turnNormal + (1  | speaker), dac %>% filter(!grepl("Robot", speaker))))
