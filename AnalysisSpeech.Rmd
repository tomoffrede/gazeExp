...
dac <- dac %>%
  mutate_at(c("turnDur", "turn", "f0mean", "robPrevf0", "gapDur", "intimMean", "intimSD", "intimMed", "IPUDur"), as.numeric) %>%
...
dac$IPUDurC <- dac$IPUDur - mean(dac$IPUDur, na.rm=TRUE)

summary(f2 <- lmer(f0mean ~ robf0c : condition + IPUDurC + (1 + condition | speaker), dac %>% filter(!grepl("Robot", speaker))))
summary(f1 <- lmer(f0mean ~ robf0c : condition  + (1 + condition | speaker), dac %>% filter(!grepl("Robot", speaker))))
...
* But intimacy does not modulate the effect of robot's f0 on human's f0 (regardless of whether we include condition in the model.)

```{r}
summary(f1 <- lmer(f0mean ~ intimc + (1 | speaker), dac %>% filter(!grepl("Robot", speaker))))

summary(f2 <- lmer(f0mean ~ intimc : robf0c : condition + (1 + condition | speaker), dac %>% filter(!grepl("Robot", speaker))))
...
summary(agree1 <- lmer(f0mean ~ robf0c : Agreec : condition + (1 +condition| speaker), d))
...
```{r}
summary(cf1 <- lmer(f0mean ~ robf0c : ConvQualc + (1 | speaker), dac %>% filter(!grepl("Robot", speaker))))
...
```{r}
summary(hl1 <- lmer(f0mean ~ robf0c : RobQualc + (1  | speaker), dac %>% filter(!grepl("Robot", speaker))))
