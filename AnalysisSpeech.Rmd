...
The model only with `robPrevf0` and without condition also doesn't give a significant effect.
...
```{r}
summary(f1 <- lmer(f0mean ~ (robPrevf0 + condition + turn)^2 + (1 | speaker), dac %>% filter(!grepl("Robot", speaker))))
```
...
The results from this regression model would suggest that people's f0 during their second baseline were predicted by the f0 of the robots they interacted with in the previous conversation. But the other regression model further below also suggests the same thing happens, except that that second model is based on f0 data from a robot that the participant hadn't interacted with yet. (There's a difference in the coefficients of `robPrevf0` depending on whether you use `GA` or `NG` as the reference condition.)
...
summary(fb1 <- lmer(f0mean ~ robPrevf0 * condition + (0 + condition | speaker), dab))
```
```{r}
ggplot(dab, aes(robPrevf0Mock, f0mean))+
  geom_point()+
  facet_wrap(~condition)
```
...
```{r}
summary(fbM <- lmer(f0mean ~ robPrevf0Mock * condition + (0+condition|speaker), dab))
```
